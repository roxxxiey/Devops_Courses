...
- Во время сборки `app` скачивает репозиторий `pallets/flask`, применяет локальный патч `patches/app.patch` и копирует из него демонстрационное веб-приложение.
- Корневой процесс `app` после стартовой настройки SSH: `python3 -m app.server`.
